...
import { type UserSqlData, initialUsersData } from '../data/userMockData';
...
export function UserManagement({ darkMode, language = 'th' }: UserManagementProps) {
  ...
  const [rawUsers, setRawUsers] = useState<UserSqlData[]>(initialUsersData as unknown as UserSqlData[]);

  const users = useMemo(() => {
    return rawUsers.map(u => {
      let currentRoleName = '';
      
      // แปลงชื่อตำแหน่ง (Role) ตามภาษา
      if (u.roleName.includes('Admin') || u.securityLevel === 'high') {
        currentRoleName = translations.roleAdmin || 'Admin';
      } else if (u.roleName.includes('Supervisor')) {
        currentRoleName = translations.roleSupervisor || 'Supervisor';
      } else {
        currentRoleName = translations.roleOfficer || 'Officer';
      }
  ...
  const handleSaveUser = (user: UserSqlData) => {
    if (editingUser) {
      setRawUsers(rawUsers.map(u => u.userId === user.userId ? user : u));
    } else {
      setRawUsers([...rawUsers, user]);
    }
  ...
  const handleDeleteUser = (userId: string) => {
    setRawUsers(rawUsers.filter(u => u.userId !== userId));
    setEditingUser(null);
  };
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const matchesRole = filterRole === 'all' || user.securityLevel === filterRole;
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900' : ''}`}>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className={`text-sm mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              {translations.showingItems} {filteredUsers.length} {translations.items}
            </p>
          </div>
              ...
              {currentItems.length === 0 ? (
                <tr>
                  <td colSpan={5} className={`px-6 py-12 text-center ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    ...
                    <p className="text-lg font-medium">{translations.noData}</p>
                  </td>
                </tr>
              ) : (
                    ...
                    <td className={`px-4 py-4 border-r ${darkMode ? 'border-gray-600 text-gray-300' : 'border-gray-200 text-gray-700'}`}>
                      <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${
                          user.securityLevel === 'high' ? 'bg-red-500' : 
                          user.securityLevel === 'medium' ? 'bg-yellow-500' : 'bg-blue-500'
                        }`}></span>
                        {user.roleName}
                      </div>
                    </td>
      ...
      {/* Modals */}
      {showAddUserModal && (
        <UserModal user={null} mode="add" darkMode={darkMode} language={language} onClose={() => setShowAddUserModal(false)} onSave={handleSaveUser} translations={translations} />
      )}
      {editingUser && (
        <UserModal user={editingUser} mode="edit" darkMode={darkMode} language={language} onClose={() => setEditingUser(null)} onSave={handleSaveUser} onDelete={handleDeleteUser} translations={translations}/>
      )}
    </div>
  );
}